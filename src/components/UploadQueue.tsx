...
export function UploadQueue() {
  ...
  const updateQueueItem = useUploadStore((state) => state.updateQueueItem);

  const cancelQueueItem = useUploadStore((state) => state.cancelQueueItem);

  const cancelAllUploads = useUploadStore((state) => state.cancelAllUploads);

  const startTransfer = useUploadStore((state) => state.startTransfer);

  const totalFiles = uploadQueue.length;

  const startCyberTransfer = useUploadStore(
    (state) => state.startCyberTransfer,
  );
  ...
  const hasAllowed = uploadQueue.some(
    (item) => item.validationState === "allowed",
  );

  const hasActiveTransfer = uploadQueue.some(
  ...
  const overallProgress =
    totalFiles === 0 ? 0 : Math.round((completedFiles / totalFiles) * 100);
    ...
    <div className="space-y-4 rounded bg-slate-800 p-4">
      <div className="flex gap-2">
        <button
          ...
          disabled={loading || hasActiveTransfer || !canTransfer}
        ...
        <button
          onClick={startCyberTransfer}
          disabled={loading || hasActiveTransfer || !canCyberTransfer}
      ...
      <div className="rounded bg-slate-900 p-4 text-sm">
        <div className="font-semibold">Scheduler Metrics</div>

        <div className="mt-2 grid grid-cols-2 gap-2">
          <div>Pending: {pendingUploads}</div>

          <div>Active: {activeUploads}</div>

          <div>Failed: {failedUploads}</div>
        </div>
      </div>
        ...
        {uploadQueue.map((item) => (
          <div key={item.id} className="rounded bg-slate-900 p-3">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-medium">{item.relativePath}</div>
